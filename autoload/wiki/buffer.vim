...
function! s:init_buffer_commands() abort " {{{1
  ...
  if b:wiki.in_journal
	...
	command! -buffer		  WikiJournalToQuarter	call wiki#journal#freq('quarter')
  endif
endfunction
...
function! s:init_buffer_mappings() abort " {{{1
  ...
  if b:wiki.in_journal
	...
	nnoremap <silent><buffer> <plug>(wiki-journal-toquarter)   :WikiJournalToQuarter<cr>
  endif
  ...
  if index(['all', 'local'], g:wiki_mappings_use_defaults) >= 0
	...
	if b:wiki.in_journal
	  call extend(l:mappings, {
			...
			\ '<plug>(wiki-journal-toquarter)': '<leader>wq',
			\})
	endif
  endif
