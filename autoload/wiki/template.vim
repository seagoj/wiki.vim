...
function! wiki#template#init() abort " {{{1
	...
	let g:wiki_journal_type = l:type
	...
	elseif l:type ==# 'q'
	call wiki#template#quarterly_summary(l:year, l:number)
	endif
endfunction
...
" }}}1
function! wiki#template#quarterly_summary(year, quarter) abort " {{{1
	let l:parser = s:summary.new()

	let l:title = copy(g:wiki_template_title_quarter)
	let l:title = substitute(l:title, '%(quarter)', a:quarter, 'g')
	let l:title = substitute(l:title, '%(year)', a:year, 'g')
